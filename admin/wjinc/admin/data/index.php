			...
			<?php
				...
				foreach($times['data'] as $var){
					if($para['actionNo']){
                        ...
                        if($this->type==72) {
                            $number = 960*(strtotime(date('Y-m-d', $date))-strtotime('2004-09-19'))/3600/24+$var['actionNo']+24902865+890919;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);	
                        }elseif($this->type==73) {
                            $number = 960*(strtotime(date('Y-m-d', $date))-strtotime('2004-09-19'))/3600/24+$var['actionNo']+24902865+875871;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);	
                        } elseif($this->type==74) {
                            // 重庆彩特殊处理
							...
							// PK10
							$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2007-11-18'))/3600/24+$var['actionNo']-1267-1273-1253;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);							
						}elseif($this->type==60){
							//广东快乐10分
							...
							//北京快乐8和PC蛋蛋
							$number = 179*(strtotime(date('Y-m-d', $date))-strtotime('2004-09-19'))/3600/24+$var['actionNo']-1292*2-1273-703;
							$sql="select * from {$this->prename}data where type={$this->type} and number='$number'";
							$data=$this->getRow($sql);							
						}elseif($this->type==21){
							//广东11选五
