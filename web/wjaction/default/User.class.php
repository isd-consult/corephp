...
class User extends WebBase{
	 ...
	 * 用户登录检查 DAVID 
	 */
	public final function loginon(){
		$username=wjStrFilter($_POST['username']);
		$password=wjStrFilter($_POST['password']);
		$sql="select * from {$this->prename}members where isDelete=0 and admin=0 and username='$username'";
		if(!$user=$this->getRow($sql, $username)){
			header('location: /');
			exit;
		}
	...
	public final function logincheck(){
		...
		if(md5($password)!=$user['password']){
			echo json_encode('密码不正确');
			
		}
		if(!$user['enable']){
			echo json_encode('您的帐号系统检测涉嫌违规操作已被暂时冻结，如有疑问请联系在线客服！');
	...
	public final function r($userxxx){
		if(!$userxxx){
			...
			$this->display('team/register1.php');
		}
		else
		{
			...
			if(!$this->getRow("select uid from {$this->prename}members where uid=?", $uid)){
				...
				$this->display('team/register1.php',0,$uid,$lid);
			}
		}
	}
	...
	public final function registers(){
		...
		if($daliuser){
				...
				$sql2="select * from {$this->prename}members where username=?";
				$parentinfo=$this->getRow($sql2, $daliuser);
				if(!$parentinfo) throw new Exception('您输入的推荐人不存在。');
				$dali=10;
		}	
		$lasttime=$this->time-24*3600;
        $regcount=$this->getValue("select count(*) from {$this->prename}members where regIP=? and regTime>{$lasttime}",ip2long($this->ip(true)));
		...
		try{
			$sql="select username from {$this->prename}members where username=?";			
			if($this->getValue($sql, $para['username'])) throw new Exception('用户"'.$para['username'].'"已经存在');
			$result=$this->insertRow($this->prename .'members', $para);
			if($result){
				$id=$this->lastInsertId();
				$this->commit();
				//return '注册成功';		
