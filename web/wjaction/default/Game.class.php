...
class Game extends WebLoginBase{
	...
	public final function numbers(){
		$type=$_GET['numbers'];
		$nowtime=date('Y-m-d');
		$todayStart = strtotime($nowtime);
		$todayEnd = $todayStart+86400;
		$sql="select time,number,data from {$this->prename}data where type=$type and time between $todayStart and $todayEnd order by time desc";
		$result=$this->getRows($sql);
		foreach($result as $var){
			$val['data'][]=	explode(",",$var['data']);
			$val['number'][]=$var['number']; 
			$val['time'][]=date('H:i',$var['time']);
		}				
	...
	public final function jsk3history(){
		...
		public final function xyk3(){
		$this->display('game/xyk3/index.php');
	}
	
	public final function xyk3history(){
		$this->display('game/xyk3/history.php');
	}
	...
	public final function gxk3history(){
		...
		public final function hbk3(){
		$this->display('game/hbk3/index.php');
	}
	
	public final function hbk3history(){
		$this->display('game/hbk3/history.php');
	}
		public final function nmgk3(){
		$this->display('game/nmgk3/index.php');
	}
	
	public final function nmgk3history(){
		$this->display('game/nmgk3/history.php');
	}
