...
//					ShowNewPageInit();
        ...
        if(date){
            ...
            if(gameId!=70||gameId!=74){
                date=moment();
                if(gameId==55 || gameId==50 || gameId==65 || gameId==21 || gameId==10 || gameId==60 || gameId==66 || gameId==11 || gameId==12 || gameId==13 || gameId==14){//次日5点前不开奖的彩种
                    if(date.hour()<=5){
    ...
    formatData : function(gameId, data) {
        ...
        } else if (gameId == 10 || gameId == 75 || gameId == 11 || gameId == 12 || gameId == 13 || gameId == 14) {
            var formatStr = "<tr><td>{turnNum}</td><td>{openTime}</td><td>{n1}</td><td>{n2}</td><td>{n3}</td><td>{total}</td><td>{totalDx}</td><td>{totalDs}</td><td>{totalxt}</td></tr>";
            for ( var i in data) {
                var d = data[i];
                ...
                htmlStr += formatStr.format({
                    ...
                    "totalDx" : totalDx,
					"totalDs" : totalDs,
					"totalxt" : History.getZFBHtmlk3([ d.n1, d.n2, d.n3 ]),
                });
            }
        } else if (gameId == 61) {
    ...
    getOpenNumHtml : function(gameId, num, openTime) {
        ...
        } else if (gameId == 10 || gameId == 11 || gameId == 12 || gameId == 13 || gameId == 14 || gameId == 75) {
    ...
    getZFBHtmlk3 : function(nums) {
        var rtStr = "";
        if (parent.isSameNum(nums)) {
            rtStr = "<span class='font_red'>豹子";
        } else if (parent.isPair(nums)) {
            rtStr = "<span class='font_blue'>对子";
        } else {
            rtStr = "三不同";
        }
        return rtStr;
