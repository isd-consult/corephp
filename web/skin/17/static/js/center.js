...
var RECH_TYPE = {
	"onlinePayment" : '在线充值',
	"alonlinePayment" : '支付宝扫码',
	"wxonlinePayment" : '微信扫码',
	"jszhuanzhang" : '建行网银转账',
	"weixin" : '微信转账',
	"alipay" : '支付宝转账',
	"cft" : '财付通转账',
	"bankTransfer" : '跨行银行卡转账',
	"unionpay" : '银行转账',
	"adminAddMoney" : '系统充值'
};
	...
	initPayType : function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				console.log(JSON.stringify(rechTypeMap));
				for ( var key in rechTypeMap) {
					...
					if (key == RECH_TYPE_KEY.ONLINE) {
						...
						if( onlineRechCfgs){ 
							...
							// console.log(JSON.stringify(rechList));
							var num=0;
			...
			for ( var i in zfbRechType.cfgList) {
				...
				$('#aw-subcol').append(
						...
						<input type="radio" name="zfbPayId" value="' + cfg.id + '" payee="' + cfg.payee + '" payee-name="' + cfg.payeeName + '" code="ZFB" pay-img="' + cfg.qrCode
								+ '" class="bankradiobtn" bank-address="' + cfg.address + '"> <i class="ico-zfb"></i>\
			...
			for ( var i in weixinRechType.cfgList) {
				...
				$('#wx-subcol').append(
						...
						<input type="radio" name="wxPayId" value="' + cfg.id + '" payee="' + cfg.payee + '" payee-name="' + cfg.payeeName + '" code="WEIXIN" pay-img="' + cfg.qrCode
								+ '" class="bankradiobtn" bank-address="' + cfg.address + '"> <i class="ico-weixin"></i>\
		...
		if (cftRechType && cftRechType.cfgList.length > 0) {
			for ( var i in cftRechType.cfgList) {
				...
				$('#cft-subcol').append(
						...
						+ '" class="bankradiobtn" bank-address="' + cfg.address + '"> <i class="ico-cft"></i>\
